...
import {
  ...
  Check, Trash2, AlertCircle, Apple, Monitor, ClipboardList,
  DollarSign,
} from "lucide-react"
...
export function ScheduleClient({ students }: { students: StudentOption[] }) {
  ...
  const fetchSlots = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/nutri/schedule?start=${start.toISOString()}&end=${end.toISOString()}`)
      if (res.ok) {
  ...
  async function updateStatus(id: string, status: string) {
    ...
    setActionLoading(id)
    try {
      await fetch(`/api/nutri/schedule?id=${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function deleteSlot(id: string) {
    ...
    try {
      await fetch(`/api/nutri/schedule?id=${id}`, { method: "DELETE" })
      await fetchSlots()
    } catch { /* ignore */ }
    setActionLoading(null)
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="flex items-center gap-2 flex-1">
          <button onClick={() => navigate(-1)} className="p-2 rounded-lg bg-white/[0.04] border border-white/[0.06] min-h-[44px] min-w-[44px] flex items-center justify-center shrink-0 hover:border-emerald-500/20 transition-colors">
            <ChevronLeft className="w-4 h-4 text-neutral-400" />
          </button>
          <button onClick={goToday} className="px-3 py-1.5 rounded-lg bg-white/[0.04] border border-white/[0.06] text-[10px] text-neutral-400 font-medium hover:border-emerald-500/20 hover:text-emerald-400 transition-colors shrink-0 min-h-[44px]">
            Hoje
          </button>
        ...
        <div className="flex items-center gap-2 justify-between sm:justify-end">
          <div className="flex rounded-lg overflow-hidden border border-white/[0.06]">
            <button onClick={() => setViewMode("day")} className={`px-3 py-1.5 text-[11px] font-medium transition-colors ${viewMode === "day" ? "bg-emerald-600/20 text-emerald-400" : "bg-white/[0.02] text-neutral-500"}`}>
              Dia
            </button>
            <button onClick={() => setViewMode("week")} className={`px-3 py-1.5 text-[11px] font-medium transition-colors ${viewMode === "week" ? "bg-emerald-600/20 text-emerald-400" : "bg-white/[0.02] text-neutral-500"}`}>
              Semana
            </button>
          </div>
          <button
            onClick={() => { setShowForm(!showForm); if (!showForm) setForm(f => ({ ...f, date: currentDate.toISOString().split("T")[0] })) }}
            className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-700 text-white text-xs font-semibold min-h-[44px] shadow-lg shadow-emerald-600/20 shrink-0 hover:from-emerald-500 hover:to-emerald-600 transition-all"
          >
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
            <div className="rounded-xl bg-white/[0.03] border border-white/[0.08] p-4 sm:p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-neutral-400 mb-1.5 block">Paciente (opcional)</label>
                  <select
                    value={form.studentId}
                    onChange={e => setForm({ ...form, studentId: e.target.value })}
                    className="w-full px-3 py-3 rounded-xl bg-white/[0.04] border border-white/[0.06] text-sm text-white focus:outline-none focus:border-emerald-500/30 min-h-[44px]"
                  >
                    <option value="">Horario livre</option>
                    {students.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs text-neutral-400 mb-1.5 block">Titulo</label>
                  <input
                    type="text"
                    value={form.title}
                    onChange={e => setForm({ ...form, title: e.target.value })}
                    placeholder="Consulta inicial, Retorno..."
                    className="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.06] text-sm text-white placeholder:text-neutral-600 focus:outline-none focus:border-emerald-500/30 min-h-[44px]"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  <label className="text-xs text-neutral-400 mb-1.5 block">Horario</label>
                  <input type="time" value={form.time} onChange={e => setForm({ ...form, time: e.target.value })}
                    className="w-full px-3 py-3 rounded-xl bg-white/[0.04] border border-white/[0.06] text-sm text-white focus:outline-none focus:border-emerald-500/30 min-h-[44px]" />
                </div>
                <div>
                  <label className="text-xs text-neutral-400 mb-1.5 block">Duracao</label>
              ...
              <div>
                <label className="text-xs text-neutral-400 mb-1.5 block">Observacoes</label>
                <textarea
                  value={form.notes}
                  onChange={e => setForm({ ...form, notes: e.target.value })}
                  placeholder="Link da chamada, orientacoes previas, observacoes..."
                  rows={3}
                  className="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.06] text-sm text-white placeholder:text-neutral-600 focus:outline-none focus:border-emerald-500/30 resize-none"
                />
              </div>
              ...
              <button onClick={createSlot} disabled={!form.date || saving}
                className="w-full py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-700 text-white text-sm font-semibold disabled:opacity-40 min-h-[44px] shadow-lg shadow-emerald-600/20 hover:from-emerald-500 hover:to-emerald-600 transition-all">
            ...
            <div className="min-w-[560px]">
              ...
              <div className="grid grid-cols-7 border-b border-white/[0.06]">
                {weekDays.map((d, i) => {
                  ...
                  return (
                    <div key={i} className={`p-2 text-center border-r border-white/[0.04] last:border-r-0 ${isToday ? "bg-emerald-600/10" : ""}`}>
                      <p className="text-[10px] text-neutral-500 uppercase">{dayNames[i]}</p>
                      <p className={`text-sm font-bold ${isToday ? "text-emerald-400" : "text-white"}`}>{d.getDate()}</p>
                    </div>
                  )
                })}
              </div>
...
function SlotCard({ slot, compact, actionLoading, onStatusChange, onDelete, onTogglePaid }: {
            ...
            >
              <div className={`mt-1 rounded-xl border p-3 space-y-2 ${statusClass}`}>
                ...
                <div className="text-[10px] opacity-70">
                  ...
                  {slot.notes && <span className="ml-1.5">- {slot.notes}</span>}
                </div>
  ...
  return (
    <motion.div
      ...
      className={`rounded-xl border p-3 space-y-2.5 ${statusClass}`}
    >
      {/* Header */}
      <div className="flex items-start justify-between gap-2">
        <div className="space-y-0.5 min-w-0">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs truncate">{slot.student?.user.name || slot.title || "Horario livre"}</span>
          </div>
        </div>
      ...
      {(slot.status === "SCHEDULED" || slot.status === "CONFIRMED") && (
        <>
          <button onClick={() => onStatusChange(slot.id, "COMPLETED")} disabled={isMutating} title="Concluir"
            className="p-1.5 rounded-lg bg-emerald-500/20 hover:bg-emerald-500/30 transition-colors disabled:opacity-40">
