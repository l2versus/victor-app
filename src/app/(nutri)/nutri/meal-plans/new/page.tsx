...
export default function NewMealPlanPage() {
  ...
  // Meals
  const [meals, setMeals] = useState<Meal[]>([createEmptyMeal()])
  ...
  async function handleSave() {
    ...
    if (meals.length === 0) return alert("Adicione pelo menos uma refeição")

    setSaving(true)
    try {
      const res = await fetch("/api/nutri/meal-plans", {
        method: "POST",
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] sm:text-[11px] text-neutral-500 uppercase tracking-[0.15em]">
              Monte as refeições e defina os macros
            </p>
          </div>
        </div>
      </motion.div>
        ...
        <div className="bg-[#060606]/90 backdrop-blur-xl border-t border-white/[0.06]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4 flex items-center justify-between gap-4">
            ...
            >
              ...
              <span className="hidden sm:inline">{saving ? "Salvando..." : "Salvar Plano"}</span>
            </button>
          </div>
        </div>
      </div>
