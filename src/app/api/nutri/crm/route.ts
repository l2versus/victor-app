...
// GET /api/nutri/crm — list leads with filters
export async function GET(req: NextRequest) {
  try {
    ...
    const [leads, stats, tempStats] = await Promise.all([
      ...
      // Pipeline stats
      Promise.all([
        prisma.lead.count({ where: { nutritionistId: nutri.id, status: "NEW" } }),
        prisma.lead.count({ where: { nutritionistId: nutri.id, status: "CONTACTED" } }),
        prisma.lead.count({ where: { nutritionistId: nutri.id, status: "TRIAL" } }),
        prisma.lead.count({ where: { nutritionistId: nutri.id, status: "NEGOTIATING" } }),
        prisma.lead.count({ where: { nutritionistId: nutri.id, status: "CONVERTED" } }),
        prisma.lead.count({ where: { nutritionistId: nutri.id, status: "LOST" } }),
      ]),
      // Temperature stats
      Promise.all([
        prisma.lead.count({ where: { nutritionistId: nutri.id, temperature: "HOT", status: { notIn: ["CONVERTED", "LOST"] } } }),
        prisma.lead.count({ where: { nutritionistId: nutri.id, temperature: "WARM", status: { notIn: ["CONVERTED", "LOST"] } } }),
        prisma.lead.count({ where: { nutritionistId: nutri.id, temperature: "COLD", status: { notIn: ["CONVERTED", "LOST"] } } }),
      ]),
    ])
    ...
    console.error("GET /api/nutri/crm error:", error)
    return NextResponse.json({ error: "Failed" }, { status: 500 })
  }
}

// POST /api/nutri/crm — create lead
export async function POST(req: NextRequest) {
  try {
    ...
    await prisma.crmActivity.create({
      data: { leadId: lead.id, action: "CREATED", details: `Paciente potencial criado via ${source || "manual"}` },
    })

    return NextResponse.json({ lead }, { status: 201 })
  } catch (error) {
    console.error("POST /api/nutri/crm error:", error)
    return NextResponse.json({ error: "Failed" }, { status: 500 })
  }
}

// PATCH /api/nutri/crm?id=xxx — update lead
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!existing || existing.nutritionistId !== nutri.id) {
    ...
    console.error("PATCH /api/nutri/crm error:", error)
    return NextResponse.json({ error: "Failed" }, { status: 500 })
  }
}

// DELETE /api/nutri/crm?id=xxx — delete lead
export async function DELETE(req: NextRequest) {
  try {
    ...
    console.error("DELETE /api/nutri/crm error:", error)
    return NextResponse.json({ error: "Failed" }, { status: 500 })
  }
}
